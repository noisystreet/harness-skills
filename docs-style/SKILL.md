...
description: >-
  ...
  release notes, recording architecture decisions, or when the user mentions
  documentation / docs / README / ADR / architecture decision / CHANGELOG / 文档 /
  技术决策.
---
...
ADR 适用于不可轻易反悔的工程决策：框架、存储、协议、部署模型、兼容性策略、关键依赖选型。

何时写：

1. 选择会约束后续实现，且 3 个月后仍需要解释「为什么」
2. 存在认真考虑过的备选方案
3. 涉及安全、数据、兼容性或运维成本的权衡

何时不写：

- 纯局部命名/文件整理
- 已有标准且无争议的格式化/lint 默认值
- 可在 issue/PR 里一次性说清、且不会成为长期约束的小决定
...
1. 状态要维护：`Superseded` 必须链接到新 ADR
2. Decision 写选择，不写长篇教程
3. Consequences 必须包含代价，不只写优点
4. 模板可参考 `project-bootstrap/templates/ADR.md`
5. 重大决策不要只留在聊天或已合并 PR 的评论里
