...
个人常用的 Cursor Agent Skills 集合。每个 skill 是一套可复用的指令，在对应场景下引导 Agent 按约定执行。
...
把需要的 skill 链到或复制到项目的 `.cursor/skills/`（在本仓库根目录执行）：

```bash
mkdir -p /path/to/your-project/.cursor/skills
# 符号链接（推荐，改一处全局生效）
ln -s "$(pwd)/rust-style" /path/to/your-project/.cursor/skills/rust-style

# 或直接复制
cp -r github-flow /path/to/your-project/.cursor/skills/
```

### 方式二：个人全局 Skills

链到 `~/.cursor/skills/`，对所有项目可用（在本仓库根目录执行）：
...
- Agent 会根据 `SKILL.md` 的 `description` 自动匹配相关 skill
